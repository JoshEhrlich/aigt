# U-Net Model Construction - 4 block model w/ attention - OLD MODEL - 544 thousand params
...
def segmentation_unet(input_size, num_classes, filter_multiplier=10, regularization_rate=0.):
    ...
    print([output])
    output = Conv2D(filters, (shape, shape), kernel_initializer="he_normal", padding = "same", strides=(2,2),  activation="relu")(output)
    print([output])
    count = 0
    ...
    #output = Conv2DTranspose(filters, (shape,shape), strides=(2,2), padding='same')(output)
    output = Conv2D(filters, (shape, shape), strides = (1,1), activation="softmax", padding="same", bias_regularizer=l1(regularization_rate))(output)
